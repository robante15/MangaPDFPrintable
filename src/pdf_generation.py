...
class PDFGeneration():
    ...
    def add_double_page(pdf: canvas.Canvas, left_page, right_page, draw_margin=True, draw_fold_line=True, UNOCUATRO=True):
        ...
        if draw_margin:
            pdf.rect(((paper_w/2)-(padding/2)-PDFGeneration.manga_size_w)*mm, ((paper_h/2) -
                    (PDFGeneration.manga_size_h/2))*mm, PDFGeneration.manga_size_w*mm, PDFGeneration.manga_size_h*mm)
        ...
        if draw_fold_line:
            pdf.line((paper_w/2)*mm, ((paper_h/2)-(PDFGeneration.manga_size_h/2))*mm,
                    (paper_w/2)*mm, ((paper_h/2)+(PDFGeneration.manga_size_h/2))*mm)

        # Agrega la pagina izquierda y derecha
        pdf.drawImage(left_page, ((paper_w/2)-(padding/2)-PDFGeneration.manga_size_w)*mm, ((paper_h/2)-(PDFGeneration.manga_size_h/2))*mm,
                    PDFGeneration.manga_size_w*mm, PDFGeneration.manga_size_h*mm)

        pdf.drawImage(right_page, ((paper_w/2)+(padding/2))*mm,
                    ((paper_h/2)-(PDFGeneration.manga_size_h/2))*mm, PDFGeneration.manga_size_w*mm, PDFGeneration.manga_size_h*mm)
    ...
    def add_single_page(pdf: canvas.Canvas, page, draw_margin=True):
        if draw_margin:
            pdf.rect(((paper_w/2)-(padding/2)-PDFGeneration.manga_size_w)*mm, ((paper_h/2)-(PDFGeneration.manga_size_h/2))*mm, PDFGeneration.manga_size_w *
                    2*mm+padding*mm, PDFGeneration.manga_size_h*mm)

        pdf.drawImage(page, ((paper_w/2)-(padding/2)-PDFGeneration.manga_size_w)*mm, ((paper_h/2)-(PDFGeneration.manga_size_h/2))*mm,
                    PDFGeneration.manga_size_w*2*mm+padding*mm, PDFGeneration.manga_size_h*mm)
        pdf.showPage()
